...
There are many intricate parameters which one can adjust during training, such as learning rate, batch size, number of samples, number of single measurements. Only with a suitable choice of parameters can one achieve a satisfactory result. For the example depicted, the size of the training data set is $8 \times 32 \times 2000$ and the variable $\alpha$ is $2$. This choice leads to an overlap of 97 percent between the exact wave function and the learned one. On the other hand, the overlap can only reach $50$ percent for a $2 \times 2$ lattice if the number of training data points is reduced to $2000$. Moreover, we observe that, only in cases where  the variables train-batch-num and sample-num both reach certain thresholds, the tomography can be successful. Otherwise, the overlap of the two wave functions will fluctuate around a very low level indefinitely. For the $2 \times 2$ lattice this threshold is reached when train-batch-num = sample-num = 1000 and higher. 

\subsubsection*{Plaquette Operator expectation values and spin flipping}

Starting from the state obtained from tomography as discussed above, one can then realize spin flipping by modifying parameters of the RBM as described in Section \hspace{0.2mm}\ref{sec4}. In order to verify that spin flipping takes us to the right excited state, we proceed by measuring the relevant plaquette operators each time. The convention for labeling plaquette operators and sites is as shown in \todo{Include figure} Figure below. Let us discuss the results in the following.

For an RBM obtained by training from a ground state by tomography, the expectation values of its four plaquette operators as well as the expected and measured energies are shown below.
\begin{figure}[!htb]
	\centering
...
Flipping the spin at lattice site number $5$ along the $y$- and $z$-directions will affect plaquettes $B$ and $D$. The corresponding expectation values are shown below.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{./images/Plaquette5.png}
	\caption{\label{tab:pl5} Expectation values of plaquette operators obtained by flipping spins at lattice site number $5$.} 
\end{figure}

Proceeding by flipping the spins of site $7$, will affect the expectation values for plaquettes $A$ and $D$, as shown in the Figure \ref{tab:pl7} below.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{./images/Plaquette7.png}
	\caption{\label{tab:pl7} Expectation values of plaquette operators obtained by flipping the spins at lattice site $7$.} 
\end{figure}

Finally, by flipping the spin at the 3rd site, which affects plaquettes A and B, we  close the circle and arrive back to the initial state, see Figure \ref{tab:pl3}.
\begin{figure}[!htb]
	\centering
	\includegraphics[width=\textwidth]{./images/Plaquette0.png}
	\caption{\label{tab:pl3} Expectation values of plaquette operators obtained by flipping spins at lattice site $3$.} 
\end{figure}
