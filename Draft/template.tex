...
where $|\Psi_{U}\rangle$ is the physical wave-function that we want to reconstruct. $\hat{U}_{b}$ is the unitary transformation which rotates the wave-function to the basis \textit{b} in which the measurement is done. One can write the same expression for the probability distribution of the measurements for the variational wave-function $|\tilde \psi_{\boldsymbol{\lambda/\mu}}\rangle$, namely $\tilde P_{b}(\boldsymbol{\sigma})$.

Then the QST problem reduces to a minimization task to find the set of parameters $\boldsymbol{\kappa}$ which minimises the total Kullback-Leibler divergence between $P_{b}(\boldsymbol{\sigma})$ and $\tilde P_{b}(\boldsymbol{\sigma})$
...
A more detailed explanation of how to perform this minimization using gradient descent can be found in \cite{Torlai_2018}. The whole process of QST was done using \texttt{NetKet} \cite{netket:2019} which has the above algorithm already built in.
...
2x2-16k.1.png is the best plot I can get so far. The parameters are, 2x2 lattice, alpha=2, training-data-size=16k, train-batch-size=n-samples=2k. The system will first be trained with lr=0.02, then with lr=0.01 if overlap reach 0.84, then with lr=0.005 if overlap reach 0.92 until overlap reach 0.97. The 0.84 and 0.92 here are only experience parameters, if too small, not efficient, too large, can not never reach.

The traning-data-size will affect the finally accuracy tomography can reach. For example, if we change it from 16k to 2k, the result will be like 2x2-2k.1.png.
