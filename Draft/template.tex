...
%     \label{fig:fig0b}
%     \end{subfigure}%
    \subfloat[]{\includegraphics[width=0.6\textwidth]{./images/Final_A_2}}\\
    \subfloat[]{\includegraphics[width=0.7\textwidth]{./images/br_zone_v2}}
...
A more detailed explanation of how to perform this minimization using gradient descent can be found in \cite{Torlai_2018}. In the present paper, we will follow the simpler approach of using the representation in Equ.\hspace{0.2mm}\ref{eq:19_0} (\ref{eq:19_1}) for the wave function, or in other words we work with a choice where $\phi_{\boldsymbol{\mu}}(\boldsymbol{\sigma}) = 0$ while the RBM $p_{\boldsymbol{\mu}}(\boldsymbol{\sigma})$ has complex parameters and thus automatically takes care of the phase. The entire process of QST was implemented using \texttt{NetKet} \cite{netket:2019} which has the above algorithm already built in.
...
There are many intricate parameters which one can adjust during training, such as learning rate, batch size, number of Metropolis samples, and number of single measurements. Only with a suitable choice of parameters and large enough training data set, can one achieve a satisfactory result. For the example depicted in Fig.\hspace{0.2mm}\ref{fig:qst}, in order to build the training data set, the exact wave function was sampled 2000 times for 256 randomly chosen basis $x, y, z$ and the hidden node density $\alpha$ was set equal to $2$. With this combinations of parameters, QST leads to an overlap of 97 percent between the exact wave function and the learned one. 
...
The software framework developed for this project is publicly available at \hyperref{https://github.com/mrnp95/TQC/}{}{}{https://github.com/mrnp95/TQC/}.
